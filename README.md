...
[![npm version](http://img.shields.io/npm/v/grunt-robocopy.svg)](https://npmjs.org/package/grunt-robocopy) [![build status](http://img.shields.io/travis/mikeobrien/grunt-robocopy.svg)](https://travis-ci.org/mikeobrien/grunt-robocopy)
...
```js
    robocopy: {
        options: {
            //Global task options here. Merged with each target options
        }
        bin: {
            options: {
                source: 'src/SomeProject.Web/bin',
        ...
        views: {
            options: {
                source: 'src/SomeProject.Web/Views',
...
## Config
Inside your `Gruntfile.js` file, add a section named `robocopy`, for the robocopy 
options. The options below mirror those of the robocopy command itself, so check 
